...
// Extracted from https://github.com/FletchMcKee/liquid

fun Modifier.liquidGlass(
    blur: Float = 5f,
    saturation: Float = 1.2f,
    refraction: Float = 0.65f,
    curve: Float = 0.08f,
    ...
    if (Build.VERSION.SDK_INT >= 31 && size.width > 0f) {
        ...
        val baseChain = android.graphics.RenderEffect.createChainEffect(colorEffect, blurEffect)
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            ...
            val shaderEffect = android.graphics.RenderEffect.createRuntimeShaderEffect(shader, "content")
            this.renderEffect = android.graphics.RenderEffect.createChainEffect(shaderEffect, baseChain).asComposeRenderEffect()
        } else {
            this.renderEffect = baseChain.asComposeRenderEffect()
        }
    }
}
